...
class Dubins:
    ...
    """
    def __init__(self, radius, point_separation, dense_straight = True):
        assert radius > 0 and point_separation > 0
        self.radius = radius
        self.point_separation = point_separation
        self.dense_straight = dense_straight
    ...
    def dubins_multi(self,targets):
        if isinstance(targets, List):
            targets = deepcopy(targets)
            idx = 0
            # 去重
            while idx < len(targets) - 1:
                if np.linalg.norm(targets[idx][:2] - targets[idx + 1][:2]) < 2 * self.radius:#self.point_separation:
                    if len(targets[idx + 1]) == 2: # 如果下一个点不包含方向（非关键点），去除下一个点
                        if idx < len(targets) - 2: # 保留最后一个点
                            targets.pop(idx + 1)
                            continue
                    if len(targets[idx]) == 2: # 如果下一个点是关键点且当前点非关键点，去除当前点
                        targets.pop(idx)
                        continue
                idx += 1    # 不去除点，判断下一个
            ...
            ptr = 0
            key_points = [] # 记录关键点位置
            while ptr < len(targets) - 2:
                if len(targets[ptr + 1]) == 3: # 中间点是关键点，跳过
                    ptr += 1
                    key_points.append(ptr)
                    continue
                t1 = targets[ptr + 1][:2] - targets[ptr][:2]
                t1 = t1 / np.linalg.norm(t1)
                t2 = targets[ptr + 2][:2] - targets[ptr + 1][:2]
                t2 = t2 / np.linalg.norm(t2)
                if abs(t1 @ t2 - 1) < 1e-3: # 如果相邻3点在一条直线上，去除中间点
                    targets.pop(ptr + 1)
                    continue
                # 中间点保留，其方向为前后两直线的方向的均值
                angle1, angle2 = np.arctan2([t1[1], t2[1]], [t1[0], t2[0]])
                angle = abs(ucommon.angle_diff(angle1, angle2) / 2)
                angle_dir = np.sign(np.cross(np.append(t1, 0), np.append(t2, 0))[2])
                targets[ptr + 1] = np.append(targets[ptr + 1], ucommon.angle_diff(angle2, angle_dir * angle))
            ...
            key_points = list(range(len(targets)))  
            
        path = np.array([[0,0]])
        info = {'straight': [0], 'sp':[0], 'length': 0.}
        for ind, target in enumerate(targets):
            if ind < (len(targets)-1):
                new_path, new_straight, new_length = self.dubins_path(target,targets[ind+1])
                path = np.vstack((np.delete(path,-1,axis=0),new_path))
                info['straight'] = info['straight'][:-1] + new_straight
                info['length'] += new_length
                if (ind + 1) in key_points:
                    info['sp'].append(len(info['straight']))
            else:
                info['sp'].append(len(info['straight']))
                return path, info
