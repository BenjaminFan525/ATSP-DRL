from utils.env_utils import Resource, Site, Job
import math
import numpy as np
...
class Device:
    def __init__(self, code, config):
        ...
        self.resource : Resource = config['resource']  # 设备携带的资源对象
        self.velocity = config['velocity']  # 设备移动速度
        self.site : Site = config['site']  # 设备当前所在站点
...
class Plane:
    def __init__(self, code, config):
        ...
        self.site : Site = config['site']  # 飞机当前所在站点
