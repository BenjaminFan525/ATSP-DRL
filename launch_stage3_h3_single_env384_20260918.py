...
RUN = WORKSPACE/'result/hkbz_train_logs/stage3_h3_single_parent_20260918_r3_env384_gpu0'
UNIT = 'hkbz-stage3-h3-single-parent-20260918-r3-env384-gpu0.service'
...
def main():
    ...
    if (m['recipe']['execution_profile'] != 'single_parent_env384_v1'
    ...
    command = ['systemd-run','--system','--no-ask-password','--unit='+UNIT,
               '--description=H3 single R0 E8 continuation env384 GPU0; experiment memory protection disabled']
    ...
    if not migration['passed'] or migration['next_epoch'] != 2:
        raise RuntimeError('Expected audited resume from local epoch 1')
