...
RUN=WORKSPACE/'result/hkbz_train_logs/stage3_h3_r0e8_fresh_20260918_r8_env384_mb192_nopost_e10_gpu0'
UNIT='hkbz-stage3-h3-r0e8-fresh-20260919-r8-env384-mb192-nopost-e10-gpu0.service'
...
def main():
    ...
    if (identity!=q['request_sha256'] or q['root']!=str(RUN) or q['after_epoch']!=4
            or q.get('target_profile')!='single_parent_env384_mb192_nopost_v1'
            or q.get('requested_minibatch')!=192 or q.get('requested_microbatch')!=192
            or q.get('requested_epochs')!=10
    ...
    command=['systemd-run','--system','--no-ask-password','--unit='+UNIT,
             '--description=Stage3 wait for epoch4 checkpoint then continue env384 mb192 nopost epochs10 GPU0']
