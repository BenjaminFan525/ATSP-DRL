...
from .utils import get_point, gen_node, get_line_cross_point, get_euler_dis, gen_edge, get_dijkstra_path, get_dijkstra_path_length
import random
import torch
...
class multiField():
    ...
    def __init__(self, num_splits, type = '#', width: Tuple = (200, 300), center_lat_lon = (40, 116), working_width = None, 
        ...
        if type == '#':
            ...
            for r in range(num_splits[1]):
                ...
                upper_points.append(get_point(np.array([upper_points[-1], self.boundary_coords[2]]), split_temp))
                split_temp = 0.5 * np.random.random() + 1 / 2 / (num_splits[1] - r + 1)
                lower_points.append(get_point(np.array([lower_points[-1], self.boundary_coords[3]]), split_temp))
            ...
            p1 = get_point(self.boundary[1], split_temp)
            split_temp = 0.6 * np.random.random() + 0.2
            p2 = get_point(self.boundary[3], split_temp)
            split_temp = 0.6 * np.random.random() + 0.2
            p3 = get_point(self.boundary[0], split_temp)
            split_temp = 0.6 * np.random.random() + 0.2
            p4 = get_point(np.array([p1, p2]), split_temp)
            self.fields_coords = [
                [
            ...
            for field_num, points in enumerate(self.fields_coords):
                self.Graph.add_nodes_from([(gen_node('bound', field_num, idx), {'coord': point}) for idx, point in enumerate(points[:-1])])
    ...
    def get_fields(self, upper_points, lower_points, rows: int, upper_node=None):
        ...
        if rows == 0:
            ...
            line1 = [get_point(np.array([upper_points[0], lower_points[0]]), split)]
            split = 0.5 * np.random.random() + 1 / 2 / (rows + 1)
            line1.append(get_point(np.array([upper_points[-1], lower_points[-1]]), split))
            line1 = np.array(line1) # 横向切分线

            # 当前横向切分线与所有纵向切分线的交点
            cur_lower_points = [line1[0]] \
                + [get_line_cross_point(line1, np.array([p1, p2])) for p1, p2 in zip(upper_points[1:-1], lower_points[1:-1])] \
                + [line1[1]]
        ...
        for cur_field_idx, points in enumerate(zip(
            ...
            # 下方2角点
            lower_node.append(gen_node('bound', field_num, 1))
            lower_node.append(gen_node('bound', field_num, 2))
            # 上方2角点
            cur_upper_node.append(gen_node('bound', field_num, 0))
            cur_upper_node.append(gen_node('bound', field_num, 3))
            
            # 连接当前地块和左侧地块
            if cur_field_idx > 0:
                self.Graph.add_weighted_edges_from([(gen_node('bound', field_num, 0), 
                                                    gen_node('bound', field_num - 1, 3), 
                                                    0)])
                self.Graph.add_weighted_edges_from([(gen_node('bound', field_num, 1), 
                                                    gen_node('bound', field_num - 1, 2), 
                                                    0)])
        # 连接当前行与上方地块
    ...
    def render(self, 
        ...
        for field in self.fields:
            field.render(ax, working_lines, entry_point, show=False, boundary=boundary, theme=theme)
        if start:
    ...
    def get_path(self, line1, line2, entry1, entry2, cartesian=False):
        if isinstance(line1, tuple):
            line1 = gen_node(*line1)
        if isinstance(line2, tuple):
            line2 = gen_node(*line2)
        ...
        if 'bound' in node1:
            if 'bound' in node2:
                ...
                last_node = [self.Graph.nodes[node2]['coord']]
                node2 = f"line_b_{entry2}-{node2.split('-')[1]}-{node2.split('-')[2]}"
        else:
            if 'bound' in node2:
                first_node = [self.Graph.nodes[node1]['coord']]
                node1 = f"line_b_{entry1}-{node1.split('-')[1]}-{node1.split('-')[2]}"
            else:
                if (node1.split('-')[1] == node2.split('-')[1]) and (entry1 == entry2):
                    # 同田块同侧
                    pass
                else:
                    first_node = [self.Graph.nodes[node1]['coord']]
        ...
        path = first_node + path + last_node
        if not cartesian:
            return path
    ...
    def edit_fields(self, car_status: list, new_ends: list = None, delete_lines:bool = True, fields_sel:list = 'all', veh_del:list = []):
        ...
        for idx, status in enumerate(car_status):
            if status['inline']:
                ...
                start_line = gen_node('line', field_idx, line_idx)
                length = get_euler_dis(self.Graph, f'start-{idx}', exit)
    ...
    def merge_row(self, merge_ids : list):
        ...
        for r in range(row):
            for c in range(col):
                ...
                if field_idx <= left:
                    self.Graph.add_nodes_from([(gen_node('bound', field_idx, idx), {'coord': point}) for idx, point in enumerate(fields_coord[:-1])])
                    if c > 0:
                        self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 0), 
                                                            gen_node('bound', field_idx - 1, 3), 
                                                            0)])
                        self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 1), 
                                                            gen_node('bound', field_idx - 1, 2), 
                                                            0)])
                    if r > 0:
                        self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 0), 
                                                            gen_node('bound', field_idx - col, 1), 
                                                            0)])
                        self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 3), 
                                                            gen_node('bound', field_idx - col, 2), 
                                                            0)])
                    ...
                    if r > 0:
                        if field_idx > left + col:
                            ...
                            self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 0), 
                                                                gen_node('bound', field_idx - col + len(merge_ids) - 1, 1), 
                                                                0)])
                            self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 3), 
                                                                gen_node('bound', field_idx - col + len(merge_ids) - 1, 2), 
                                                                0)])
                        elif c == mcol_left:
                            self.Graph.add_weighted_edges_from([(gen_node('bound', field_idx, 0), 
                                                                gen_node('bound', field_idx - col + len(merge_ids) - 1, 1), 
    ...
    def merge_col(self, merge_ids : list):
        ...
        for r in range(row):
            for c in range(col):
                ...
                if field_idx <= left:
                    self.Graph.add_nodes_from([(gen_node('bound', inv_transpose(field_idx), idx), {'coord': point}) for idx, point in enumerate(fields_coord[:-1])])
                    if c > 0:
                        self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(0)), 
                                                            gen_node('bound', inv_transpose(field_idx - 1), inv_transpose(3)), 
                                                            0)])
                        self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(1)), 
                                                            gen_node('bound', inv_transpose(field_idx - 1), inv_transpose(2)), 
                                                            0)])
                    if r > 0:
                        self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(0)), 
                                                            gen_node('bound', inv_transpose(field_idx - col), inv_transpose(1)), 
                                                            0)])
                        self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(3)), 
                                                            gen_node('bound', inv_transpose(field_idx - col), inv_transpose(2)), 
                                                            0)])
                elif field_idx > right:
                    field_idx -= len(merge_ids) - 1
                    self.Graph.add_nodes_from([(gen_node('bound', inv_transpose(field_idx), idx), {'coord': point}) for idx, point in enumerate(fields_coord[:-1])])
                    ...
                    if r > 0:
                        if field_idx > left + col:
                            ...
                            self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(0)), 
                                                                gen_node('bound', inv_transpose(field_idx - col + len(merge_ids) - 1), inv_transpose(1)), 
                                                                0)])
                            self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(3)), 
                                                                gen_node('bound', inv_transpose(field_idx - col + len(merge_ids) - 1), inv_transpose(2)), 
                                                                0)])
                        elif c == mcol_left:
                            self.Graph.add_weighted_edges_from([(gen_node('bound', inv_transpose(field_idx), inv_transpose(0)), 
                                                                gen_node('bound', inv_transpose(field_idx - col + len(merge_ids) - 1), inv_transpose(1)), 
