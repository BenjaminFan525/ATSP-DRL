...
SUITE=${1:?Usage: bash launch_stage3_sampling_audit_half.sh ABSOLUTE_NEW_SUITE_DIRECTORY}
test -f "$SUITE/manifest.json"
test ! -e "$SUITE/status.json"
CODE_ROOT="$SUITE/source"
test -f "$CODE_ROOT/onpolicy/scripts/train/run_stage3_sampling_audit.py"
for gpu in 0 1 2 3; do
  occupied=$(nvidia-smi -i "$gpu" --query-compute-apps=pid --format=csv,noheader,nounits)
  if [[ -n "$occupied" ]]; then
    echo "GPU $gpu has unrelated compute processes: $occupied; refusing collision" >&2
    exit 1
  fi
done
mkdir -p "$SUITE/logs"
UNIT="hkbz-s3sampling-$(basename "$SUITE")"
systemd-run --user --unit="$UNIT" \
  ...
  --property=TimeoutStopSec=30 --property=RuntimeMaxSec=93600 \
  ...
  "$CODE_ROOT/onpolicy/scripts/train/run_stage3_sampling_audit.py" run --manifest "$SUITE/manifest.json"
