...
# One complete train+local-validation workflow per physical GPU.  Every lane
# owns disjoint SMT siblings and binds allocations to its GPU-local NUMA node.

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd -- "${SCRIPT_DIR}/../../.." && pwd)"
PYTHON="${PYTHON:-/data/fanyx/conda/envs/maia-hkbz-cu124-20260903/bin/python3.11}"
PHASE="${PHASE:-canary}"
RUN_TAG="${RUN_TAG:-stage3_credit_factorial_4090_20260903_r2}"
START="${START:-1}"
ALLOW_BUSY_GPU="${ALLOW_BUSY_GPU:-0}"
UNIT_PREFIX="${UNIT_PREFIX:-hkbz-s3cf-r2}"
SUITE_DIR="${ROOT_DIR}/result/hkbz_train_logs/${RUN_TAG}"
PREPARER="${SCRIPT_DIR}/prepare_stage3_credit_factorial.py"
RUNNER="${SCRIPT_DIR}/run_stage3_credit_manifest.py"
...
ARMS=(W000 W100 W010 W001 W110 W101 W011 W111)
CPU_SETS=(
  "0-6,64-70"
  "7-13,71-77"
  "14-20,78-84"
...
echo "[Prepared] ${PHASE}: 8 immutable manifests passed command and affinity checks."
if [[ "${START}" == "0" ]]; then
  exit 0
fi
...
for gpu in 0 1 2 3 4 5 6 7; do
  ...
  systemd-run --user --collect --unit="${unit}" --same-dir \
    ...
    --setenv="MPLCONFIGDIR=/tmp/hkbz-mpl-g${gpu}" \
    --property=Type=exec \
    --property=Restart=no \
...
echo "[Started] ${PHASE} has 8 independent train+validation services; no automatic retry."
