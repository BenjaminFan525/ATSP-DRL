...
# Stage3-R3 shared-encoder causal screen on GPU1:
#   E0 frozen B1 control
#   E1 immediate shared adaptation at LR scale 0.02
#   E2 one frozen epoch, then 0.01 -> 0.025 -> 0.05
#   E3 E2 plus shared-encoder-only PCGrad
# All lanes use seed1, graph=1000, disjoint physical cores, one shared
# Valid60 service, and one serialized PPO-update lock.  Once all four lanes
# and their final Valid60 evaluations terminate, the autopilot releases the
# evaluator and launches full-joint IGA-180 -> nested IGA-1800 on tune60.
...
AUTOPILOT="${ROOT_DIR}/onpolicy/scripts/train/stage3_encoder_wave1_autopilot.py"

GPU=1
PROFILE="${PROFILE:-encoder_wave1}"
START="${START:-0}"
WAIT_FOR_COMPLETION="${WAIT_FOR_COMPLETION:-0}"
METHOD_FILTER="${METHOD_FILTER:-all}"
AUTO_ANALYZE="${AUTO_ANALYZE:-1}"
RUN_TAG="${RUN_TAG:-stage3_encoder_wave1_20260824_r3}"
UNIT_PREFIX="${UNIT_PREFIX:-hkbz-s3enc-r3-g1}"
...
JOINT_IGA_DATASET="${JOINT_IGA_DATASET:-${VALIDATION_DIR}}"
JOINT_IGA_OUTPUT_ROOT="${JOINT_IGA_OUTPUT_ROOT:-${SUITE_DIR}/joint_iga_tune60}"
JOINT_IGA_CPUS="${JOINT_IGA_CPUS:-36-71,108-143}"
JOINT_IGA_WORKERS="${JOINT_IGA_WORKERS:-60}"
...
# CPU x and x+72 are SMT siblings of one physical core.  Each service owns
# whole cores and no logical or physical core overlaps another service.
TRAIN_CPUS=(
  "36-42,108-114"
  "43-49,115-121"
  "50-56,122-128"
...
case "${PROFILE}" in
  ...
  encoder_wave1) MAX_EVAL_CASES=60 ;;
  *) echo "[Error] PROFILE must be memory_canary, canary, or encoder_wave1." >&2; exit 1 ;;
esac
...
print("[CPU] GPU1 NUMA partition: validator=16, four trainers=14 logical CPUs each")
...
prepare_manifests() {
  ...
  for lane in 0 1 2 3; do
    method="${METHODS[${lane}]}"; manifest="$(manifest_for_lane "${lane}")"
    ...
    PYTHON="${PYTHON}" "${PYTHON}" "${MANIFEST_CHECKER}" "${manifest}" --check-only
  done
}
...
launch_evaluator() {
  ...
  unit="$(unit_name eval)"; socket="/tmp/${unit}.sock"
...
wait_evaluator() {
  ...
  unit="$(unit_name eval)"; deadline=$((SECONDS + 1200))
...
' "${socket}"; then
      echo "[Ready] ${unit}.service workers=10 CPUs=${VAL_CPUS}"; return 0
    fi
    sleep 2
  done
  echo "[Error] Shared evaluator readiness timeout." >&2; return 1
}
...
launch_trainers() {
  ...
  for lane in 0 1 2 3; do
    selected_lane "${lane}" || continue
    method="${METHODS[${lane}]}"; unit="$(unit_name "${method}")"
    log="${SUITE_DIR}/service_logs/${unit}.log"; cpu_set="${TRAIN_CPUS[${lane}]}"
...
validate_completions() {
  ...
  for lane in 0 1 2 3; do
    selected_lane "${lane}" || continue
    method="${METHODS[${lane}]}"; status="$(experiment_dir_for_lane "${lane}")/run1/run_status.json"
    ...
    jq -e '
      ...
      and ((.actor_update_health.step_completion_rate // 0) | tonumber) >= 0.9
...
launch_autopilot() {
  local unit log
  unit="${UNIT_PREFIX}-encoder-wave1-autopilot"
  log="${SUITE_DIR}/service_logs/${unit}.log"
  systemd-run --user --unit="${unit}" --same-dir \
    ...
    --property="StandardOutput=append:${log}" --property="StandardError=append:${log}" \
    "${PYTHON}" -u "${AUTOPILOT}" --suite "${SUITE_DIR}" \
      --results-root "${RESULTS_ROOT}" --unit-prefix "${UNIT_PREFIX}" \
      --joint-iga-dataset "${JOINT_IGA_DATASET}" \
      --joint-iga-output-root "${JOINT_IGA_OUTPUT_ROOT}" \
      --joint-iga-python "${PYTHON}" --joint-iga-workers "${JOINT_IGA_WORKERS}" \
      --joint-iga-cpu-set "${JOINT_IGA_CPUS}" >/dev/null
  echo "[Launch] ${unit}.service applies strict E0-E3 gates, then launches full-joint IGA-180/1800; no fallback Formal."
}

main() {
  local socket
  [[ "${METHOD_FILTER}" == all || " ${METHODS[*]} " == *" ${METHOD_FILTER} "* ]] || {
    echo "[Error] METHOD_FILTER must be all/E0/E1/E2/E3." >&2; exit 1;
  }
  [[ "${AUTO_ANALYZE}" == 0 || "${AUTO_ANALYZE}" == 1 ]] || {
    echo "[Error] AUTO_ANALYZE must be 0 or 1." >&2; exit 1;
  }
  if [[ "${PROFILE}" == encoder_wave1 && "${AUTO_ANALYZE}" == 1 && "${METHOD_FILTER}" != all ]]; then
    echo "[Error] Automatic encoder selection requires all E0-E3 arms." >&2; exit 1
  fi
  prepare_manifests
  if [[ "${START}" != 1 ]]; then
    echo "[Ready] Prepared Stage3 encoder ${PROFILE} manifests in ${SUITE_DIR}/commands"; return 0
  fi
  verify_hardware; require_fresh_outputs
  trap stop_profile ERR INT TERM
  socket="$(launch_evaluator)"; wait_evaluator "${socket}"; launch_trainers "${socket}"
  if [[ "${WAIT_FOR_COMPLETION}" == 1 ]]; then
    wait_for_trainers; validate_completions
    systemctl --user stop "$(unit_name eval).service" >/dev/null 2>&1 || true
  elif [[ "${PROFILE}" == encoder_wave1 && "${AUTO_ANALYZE}" == 1 ]]; then
    launch_autopilot
  fi
  trap - ERR INT TERM
  echo "[Started] Stage3 encoder ${PROFILE} GPU1 methods=${METHOD_FILTER} graph=${GRAPHS_PER_FORWARD} shared_eval=${socket}"
}
